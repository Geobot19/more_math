...
{
public:
	//contains the actual values (it's public so that you can use the abilities of arrays)
	bool* core = NULL;
	...
	//rezizes the array (truncates if the size is smaller than the original size
	...
	void add_2(int val)
	{
		...
		int o_size = size, required_i = (size >= val_1.size ? size : val_1.size);
		...
		for (int i = 0; i <= required_i; i++)
		{
			if (i > o_size)
			{
				core[i] = val_1.core[i];
			}
			else if (core[i] && val_1.core[i])
			{
				core[i] = false;
				core[i + 1] = true;
			}
			...
			{
				core[i] = core[i] || val_1.core[i];
			}
		}
	}
	void add_2(binary val)
	{
		int o_size = size, required_i = (size >= val.size ? size : val.size);
		...
		for (int i = 0; i <= required_i; i++)
		{
			...
			{
				core[i] = core[i] || val.core[i];
			}
		}
	}
	void multiply_2(int val)
	{
		if (val == 0)
		{
			set(0);
...
binary fibonacci_hash(string original, int loop)
{
